...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "STRESS — A private room for two people" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "STRESS — A private room for two people" },
      {
        ...
        <section className="rise pt-10 sm:pt-16">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button asChild size="lg" className="h-12 text-base">
              <Link to={session ? "/rooms" : "/auth"} search={session ? undefined : { mode: "signup" }}>
                {session ? t("onboarding.done") : t("landing.start")}
              </Link>
            </Button>
            {!session && !loading ? (
